...
import dataset.tusimple_dataset as ds
...
def main():
    ...
    # Data loading code
    train_dataset = ds.LaneDataset(args, '/home/yuliangguo/Datasets/tusimple',
                                   'list/tusimple/train.json', data_aug=True)
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size,
                                               shuffle=True, num_workers=args.workers, pin_memory=False, drop_last=True)

    val_dataset = ds.LaneDataset(args, '/home/yuliangguo/Datasets/tusimple/',
                                 'list/tusimple/val.json', data_aug=False)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target, idx) in enumerate(train_loader):
        ...
        if (i + 1) % args.print_freq == 0:
            print((
                      'Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t' 'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' 'Data {data_time.val:.3f} ({data_time.avg:.3f})\t' 'Loss {loss.val:.4f} ({loss.avg:.4f})\t'.format(
...
def validate(val_loader, model, criterion, iter, evaluator, evaluate=False):
    ...
    directory = 'predicts/tusimple/output'
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    if not os.path.exists('trained_tusimple'):
        os.makedirs('trained_tusimple')
    filename = os.path.join('trained_tusimple', '_'.join((args.snapshot_pref, args.method.lower(), filename)))
    torch.save(state, filename)
    if is_best:
        best_name = os.path.join('trained_tusimple',
                                 '_'.join((args.snapshot_pref, args.method.lower(), 'model_best.pth.tar')))
